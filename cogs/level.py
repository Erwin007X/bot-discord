...
class Levelsys(commands.Cog):
  ...
  async def on_ready(self):
    print('level_cum Cog has been loaded\n---------------------------')
    ...
    if not message.content.startswith("?"):
      ...
      if not message.author.bot:
        ...
        if str(message.author.id) in data:
          ...
          if new_level > lvl:
            await message.channel.send(f"เอาเรื่อง {message.author.mention} เลเวล {new_level}!!!")
            ...
            for i in range(len(level)):
              if new_level == level_num[i]:
                ...
                mbed = discord.Embed(title=f"{message.author} ได้รับยศ **{level[i]}**!", color = message.author.colour)
  ...
  async def rank(self, ctx: commands.Context, user: Optional[discord.Member]):
    ...
    ## Rank card
    background = Editor(f"./image/levels/zIMAGE.png")
    ...
    ima = Editor("./image/levels/zBLACK.png")
    ...
    card = File(fp=background.image_bytes, filename="./image/levels/zCARD.png")
